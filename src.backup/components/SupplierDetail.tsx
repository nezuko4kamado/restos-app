...
export default function SupplierDetail({
  ...
  const calculateStats = () => {
    ...
    invoices.forEach(inv => {
      ...
      monthlySpending[monthKey] = (monthlySpending[monthKey] || 0) + (inv.amount || 0);
    });
    ...
    return {
      ...
      monthlySpending,
      topProducts,
      averageInvoice: invoices.length > 0 ? totalSpent / invoices.length : 0,
    };
  };
                          ...
                          <div className="pt-2 border-t">
                            <div className="flex items-center gap-2">
                              {priceChange.isIncrease ? (
                                <div className="flex items-center gap-1 text-red-600 font-semibold">
                                  ...
                                  <span>+{Math.abs(priceChange.changePercent).toFixed(1)}%</span>
                                </div>
                            ...
                            {priceChange.isIncrease && (
                              <p className="text-xs text-red-600 mt-1">
                                {t.priceIncreased} €{Math.abs(priceChange.change).toFixed(2)}
                              </p>
                            )}
                            {priceChange.isDecrease && (
                              <p className="text-xs text-green-600 mt-1">
                                {t.priceDecreased} €{Math.abs(priceChange.change).toFixed(2)}
                              </p>
                            )}
                          </div>
                        )}
                  ...
                  <div key={year} className="space-y-4">
                    ...
                    {Object.keys(groupedInvoices[Number(year)])
                      ...
                      .map((month) => {
                        ...
                        const monthTotal = monthInvoices.reduce((sum, inv) => sum + (inv.amount || 0), 0);
                          ...
                          <div key={month} className="space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-medium">
                                {t.totalAmount}: €{monthTotal.toFixed(2)}
                              </span>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
