...
export const exportProductsToExcel = (products: Product[], suppliers: Supplier[]) => {
  ...
  const data = products.map(product => ({
    ...
    'Prezzo Finale (€)': (product.price * (1 - (product.discountPercent || product.discount || 0) / 100)).toFixed(2),
...
export const exportProductsToPDF = (products: Product[], suppliers: Supplier[]) => {
  ...
  const tableData = products.map(product => [
    ...
    `€${(product.price * (1 - (product.discountPercent || product.discount || 0) / 100)).toFixed(2)}`
  ]);
  ...
  doc.text(`Valore Totale: €${totalValue.toFixed(2)}`, 14, finalY + 17);
...
export const exportSuppliersToExcel = (
  ...
  const summaryData = suppliers.map(supplier => {
    ...
    return {
      ...
      'Media Fattura (€)': supplierInvoices.length > 0 ? (totalSpent / supplierInvoices.length).toFixed(2) : '0.00',
  ...
  suppliers.forEach(supplier => {
    ...
    supplierInvoices.forEach(invoice => {
      detailedData.push({
        ...
        'Importo (€)': invoice.amount.toFixed(2),
...
export const exportSuppliersToPDF = (
  ...
  suppliers.forEach((supplier, index) => {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(`Totale Speso: €${totalSpent.toFixed(2)} | Fatture: ${supplierInvoices.length} | Prodotti: ${supplierProducts.length}`, 14, currentY);
    currentY += 10;
  ...
  doc.text(`Spesa Totale: €${totalSpent.toFixed(2)}`, 14, finalY + 24);
...
export const exportStatisticsToExcel = (
  ...
  const summaryData = [
    { 'Metrica': 'Spesa Totale', 'Valore': `€${stats.totalSpent.toFixed(2)}` },
    { 'Metrica': 'Media Mensile', 'Valore': `€${stats.averageMonthlySpent.toFixed(2)}` },
    { 'Metrica': 'Numero Fatture', 'Valore': stats.totalInvoices.toString() },
    { 'Metrica': 'Importo Medio Fattura', 'Valore': `€${stats.averageInvoiceAmount.toFixed(2)}` },
    { 'Metrica': 'Mese con Spesa Più Alta', 'Valore': stats.highestMonth.month },
    { 'Metrica': 'Importo Mese Più Alto', 'Valore': `€${stats.highestMonth.amount.toFixed(2)}` }
  ];
  ...
  const monthlyData = stats.monthlyData.map(m => ({
    ...
    'Importo Totale (€)': m.amount.toFixed(2),
    'Importo Medio (€)': (m.amount / m.count).toFixed(2)
  }));
  ...
  const supplierData = stats.supplierBreakdown.map(s => ({
    ...
    'Importo (€)': s.amount.toFixed(2),
    'Percentuale (%)': s.percentage.toFixed(1)
  }));
...
export const exportStatisticsToPDF = (
  ...
  const summaryData = [
    ['Spesa Totale', `€${stats.totalSpent.toFixed(2)}`],
    ['Media Mensile', `€${stats.averageMonthlySpent.toFixed(2)}`],
    ['Numero Fatture', stats.totalInvoices.toString()],
    ['Importo Medio Fattura', `€${stats.averageInvoiceAmount.toFixed(2)}`],
    ['Mese con Spesa Più Alta', `${stats.highestMonth.month} - €${stats.highestMonth.amount.toFixed(2)}`]
  ];
