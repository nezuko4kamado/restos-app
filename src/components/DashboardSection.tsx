...
const safeToFixed = (value: number | undefined | null, decimals: number = 2): string => {
  if (value === undefined || value === null || isNaN(Number(value))) return '0.' + '0'.repeat(decimals);
...
export default function DashboardSection({ products, suppliers, orders, priceAlerts }: DashboardSectionProps) {
  const { t } = useLanguage();
  
  const stats = useMemo(() => {
    // Calculate total value of products
    const totalProductValue = products.reduce((sum, p) => sum + ((Number(p.price) || 0) * (Number(p.stock_quantity) || 0)), 0);
    
    // Calculate average product price
    const avgProductPrice = products.length > 0 ? products.reduce((sum, p) => sum + (Number(p.price) || 0), 0) / products.length : 0;
    ...
    const priceIncreases = recentPriceChanges.filter(p => (Number(p.price) || 0) > (Number(p.original_price) || Number(p.price) || 0)).length;
    const priceDecreases = recentPriceChanges.filter(p => (Number(p.price) || 0) < (Number(p.original_price) || Number(p.price) || 0)).length;
              ...
              {unacknowledgedAlerts.slice(0, 5).map(alert => (
                <div key={alert.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-orange-200">
                  <div className="flex items-center gap-3">
                    {(Number(alert.change_percent) || 0) > 0 ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        €{safeToFixed(alert.old_price, 2)} → €{safeToFixed(alert.new_price, 2)}
                      </p>
                    </div>
                  </div>
                  <Badge className={(Number(alert.change_percent) || 0) > 0 ? 'bg-red-500' : 'bg-green-500'}>
                    {(Number(alert.change_percent) || 0) > 0 ? '+' : ''}{safeToFixed(alert.change_percent, 1)}%
                  </Badge>
                </div>
              ))}
            </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{t('price')}: €{safeToFixed(stats.avgProductPrice, 2)}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{t('totalAmount')}: €{safeToFixed(stats.totalOrdersValue, 2)}</p>
              </div>
