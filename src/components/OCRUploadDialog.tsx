    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (file.type === 'application/pdf') {
          ...
          // Fix: Remove type parameter and access data correctly
          result = await extractDataFromImage(file);
        } else {
        ...
        console.log('📊 OCR Result structure:', result);

        // Fix: Access products and supplier from result.data
        if (type === 'products' && result.success && result.data?.products) {
          allProducts.push(...result.data.products);
          if (result.data.supplier && !supplierInfo) {
            supplierInfo = {
              id: generateUUID(),
              name: result.data.supplier.name || result.data.supplier_name || 'Unknown Supplier',
              email: result.data.supplier.email || '',
              phone: result.data.supplier.phone || '',
              address: result.data.supplier.address || ''
            };
          }
        } else if (type === 'suppliers' && result.suppliers) {
          allSuppliers.push(...result.suppliers);
      ...
      if (type === 'products' && allProducts.length > 0) {
        ...
        console.error('❌ No data extracted from any file');
        toast.error(t('error'));
      }
    } catch (error) {
