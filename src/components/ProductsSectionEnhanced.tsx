...
function ProductCard({ 
  ...
  const discountedPrice = (product.discounted_price !== undefined && product.discounted_price !== null && product.discounted_price > 0)
...
export default function ProductsSectionEnhanced({ 
  ...
  // Ref per pulsante camera (ref.click() è l'unico modo affidabile su Android TWA)
        ...
        if (existingProduct) {
          {
            ...
            if (priceActuallyChanged) {
              ...
              const sign = diff > 0 ? '+' : '';
              const currency2 = settings.defaultCurrency || 'EUR';
              toast.info(
                `💰 ${existingProduct.name}: ${formatPrice(oldPrice, currency2)} → ${formatPrice(newPrice, currency2)} (${sign}${formatPrice(diff, currency2)})`,
                { duration: 6000 }
              );
            }
          }
  ...
  return (
    <Card className="backdrop-blur-xl bg-white/80 dark:bg-slate-900/80 border-white/20 dark:border-slate-700/20 shadow-xl w-full max-w-full overflow-x-hidden">
      <CardHeader className="px-3 sm:px-6 overflow-visible">
        <div className="flex flex-col gap-3 sm:gap-4">
          ...
          <div className="grid grid-cols-2 gap-2 w-full">
            ...
            <Button
              ...
              className="w-full min-h-[40px] sm:min-h-[44px] text-[11px] sm:text-sm px-2 sm:px-3 border-2 border-blue-300 text-blue-700 hover:border-blue-500 hover:bg-blue-50 dark:border-blue-700 dark:text-blue-400"
            >
            ...
            {/* Galleria: label+input va bene senza capture */}
            <input
