  ...
  try {
    ...
    if (insertError) {
      if (insertError.code === '23505') {
        console.log('ℹ️ [REGISTER] Subscription record already exists (concurrent insert)');
        return;
      }
      console.warn('⚠️ [REGISTER] Could not create subscription record:', insertError.message);
      return;
    }

    console.log('🟢 [REGISTER] Created fallback subscription record for user:', userId);
  } catch (err) {
    console.warn('⚠️ [REGISTER] Could not ensure subscription record:', err);
  }
}
...
export function RegisterForm({ onSuccess, onSwitchToLogin }: RegisterFormProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle "Database error" specifically — the user may have been created
      // but the trigger that creates user_subscriptions failed.
      if (signUpError && signUpError.message.includes('Database error')) {
        console.warn('⚠️ [REGISTER FORM] Database error during signup — attempting recovery via sign-in...');

        // Try signing in: if the user was actually created, this will succeed
        const { data: signInData, error: signInError } = await signIn(email, password);

        if (!signInError && signInData?.user) {
          console.log('🟢 [REGISTER FORM] Recovery sign-in succeeded for user:', signInData.user.id);

          // Ensure subscription record exists (client-side fallback)
          await ensureSubscriptionRecord(signInData.user.id);
        ...
        console.error('🔴 [REGISTER FORM] Recovery sign-in also failed:', signInError);
        const errorMessage = t('databaseErrorRegistration') || 'Database error during registration. Please try again in a moment.';
      ...
      console.error('🔴 [REGISTER FORM] Caught error:', err);

      const errorMessage =
        err instanceof Error ? err.message : (t('registrationError') || 'Error during registration');
