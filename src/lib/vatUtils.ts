 ...
 * 🔥 CRITICAL: Added extensive logging to debug VAT calculations
 */
export function calculatePriceWithVAT(price: number, vatRate: number): number {
  const result = price * (1 + vatRate / 100);
  
  console.log('💰 [VAT CALC] calculatePriceWithVAT called:');
  console.log('  - Base price:', price.toFixed(2), '€');
  console.log('  - VAT rate:', vatRate, '%');
  console.log('  - VAT amount:', (price * (vatRate / 100)).toFixed(2), '€');
  console.log('  - Final price (with VAT):', result.toFixed(2), '€');
  console.log('  - Formula: price × (1 + vatRate/100) =', price, '× (1 +', vatRate, '/100) =', price, '×', (1 + vatRate / 100), '=', result.toFixed(2));
