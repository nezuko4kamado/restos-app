...
export default function ContactUs() {
  ...
  const handleBack = () => {
    // Check if we're in hash routing mode
    const isHashRoute = window.location.hash.startsWith('#/');
    
    if (isHashRoute) {
      // From hash routing, always redirect to non-hash routes using full page reload
      if (user) {
        // User is logged in, go to home page (without hash)
        window.location.href = '/';
      } else {
        // User is not logged in, go to login page (without hash)
        window.location.href = '/login';
      }
    } else {
      // From normal routing, use navigate
      if (user) {
        navigate('/');
      } else {
        navigate('/login');
